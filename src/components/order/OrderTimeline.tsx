          ...
          <li key={stage} className="flex gap-3">
            <div className="flex flex-col items-center">
              <span
                className={cn('grid h-7 w-7 shrink-0 place-items-center rounded-full border-2 text-[11px] font-bold',
                  done ? 'border-brand-600 bg-brand-600 text-white' : 'border-line bg-surface text-subtle')}
                aria-hidden="true"
              >
                {done ? <CheckIcon className="h-3.5 w-3.5" /> : index + 1}
              </span>
              {index < stages.length - 1 ? (
                <span className={cn('w-0.5 flex-1', index < currentIndex ? 'bg-brand-600' : 'bg-line')} />
              ) : null}
            </div>
            ...
            <div className={cn('pb-5', index === stages.length - 1 && 'pb-0')}>
              <p className={cn('text-sm font-semibold', done ? 'text-ink' : 'text-subtle')}>
                ...
                {index === currentIndex ? <span className="badge ml-2 bg-brand-100 text-brand-800">Current</span> : null}
              </p>
