...
export function Pagination({
  ...
  const base = 'inline-flex h-11 min-w-[2.75rem] items-center justify-center rounded-full border px-3 text-sm font-medium';

  return (
    <nav aria-label="Pagination" className="mt-8 flex flex-wrap items-center justify-center gap-1.5">
      {page > 1 ? (
        <Link href={buildHref(page - 1)} rel="prev" className={cn(base, 'border-line bg-surface hover:bg-raise')} aria-label="Previous page">
      ...
      {pages.map((entry, index) =>
        entry === 'gap' ? (
          ...
          <Link
            ...
            className={cn(base, entry === page ? 'border-clay-600 bg-ink text-white' : 'border-line bg-surface hover:bg-raise')}
          >
            {entry}
          </Link>
        ),
      ...
      {page < totalPages ? (
        <Link href={buildHref(page + 1)} rel="next" className={cn(base, 'border-line bg-surface hover:bg-raise')} aria-label="Next page">
