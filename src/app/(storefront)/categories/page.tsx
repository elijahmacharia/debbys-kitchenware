            ...
            <section key={category.id} className="card p-4">
              <Link href={`/category/${category.slug}`} className="group flex items-start gap-3">
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-1 text-base font-semibold text-ink group-hover:text-ink">
                    {category.name}
              ...
              {category.children.length > 0 ? (
                <ul className="mt-3 flex flex-wrap gap-1.5 border-t border-line pt-3">
                  {category.children.map((child) => (
                    <li key={child.id}>
                      <Link href={`/category/${child.slug}`} className="inline-block rounded-full border border-line px-2.5 py-1 text-xs text-ink hover:border-ink hover:bg-raise">
